...
public class fornecedor extends JDialog {
	 ...
	 */
	public fornecedor() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(fornecedor.class.getResource("/img/pngegg.png")));
		...
		scrollPaneNome.setBounds(129, 53, 240, 65);
		getContentPane().add(scrollPaneNome);
		
				listaNome = new JList();
				listaNome.addKeyListener(new KeyAdapter() {
					@Override
		...
		txtNome.setBounds(129, 36, 240, 20);
		...
		txtFone.setBounds(129, 134, 240, 16);
		...
		btnAdd.setIcon(new ImageIcon(fornecedor.class.getResource("/img/addblackcircularbutton_104741 (1).png")));
		...
		btnEditar.setIcon(new ImageIcon(fornecedor.class.getResource("/img/pencileditblackcircularbuttoninterfacesymbol_104762 (4).png")));
		...
		btnExcluir.setIcon(new ImageIcon(fornecedor.class.getResource("/img/crossdeleteblackcircularbutton_79721.png")));
		...
		btnLimpar.setIcon(new ImageIcon(fornecedor.class.getResource("/img/pngegg (3).png")));
		...
		lblNewLabel_1.setBounds(57, 39, 46, 14);
		...
		lblNewLabel_4.setBounds(57, 132, 46, 14);
		...
		txtCNPJ.setBounds(129, 101, 240, 20);
	...
	private void ListarCliente() {
		...
		String readlistaCliente = "select * from fornecedor where nome like '" + txtNome.getText() + "%'" + "order by nome";
		try {
			con = dao.conectar();
	...
	private void BuscarNomeCliente() {
		...
		if (linha >= 0) {
			String readlistaCliente = "select * from fornecedor where nome like '" + txtNome.getText() + "%'"
			...
			try {
				...
				if (rs.next()) {
					...
					JOptionPane.showMessageDialog(null, "Fornecedor0, Inexistente");
				}
				con.close();
			} catch (Exception e) {
				System.out.println(e);
	...
	private void Add() {
		if (txtNome.getText().isEmpty()) {
			...
			JOptionPane.showMessageDialog(null, "Insira o Telefone do Fornecedor");
			txtCNPJ.requestFocus();
		} else if (txtFone.getText().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Insira o Telefone do Fornecedor");
			txtFone.requestFocus();
		} else if (txtCep.getText().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Insira o Cep do Fornecedor");
			txtCep.requestFocus();
		} else if (txtEndereco.getText().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Insira o Endereço do Fornecedor");
			txtEndereco.requestFocus();
		} else if (txtNumero.getText().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Insira o Numero do Fornecedor");
			txtNumero.requestFocus();
		} else if (txtBairro.getText().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Insira o Bairro do Fornecedor");
			txtBairro.requestFocus();
		} else if (txtCidade.getText().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Insira o Cidade do Fornecedor");
			txtCidade.requestFocus();
		} else if (cboUf.getSelectedItem().equals("")) {
			JOptionPane.showMessageDialog(null, "Insira o UF do Fornecedor");
			cboUf.requestFocus();
		} else {
			String create = "insert into fornecedor (nome,cnpj,fone,cep,endereco,numero,complemento,bairro,cidade,uf) values (?,?,?,?,?,?,?,?,?,?)";
			try {
				con = dao.conectar();
	...
	private void Editar() {
		if (txtNome.getText().isEmpty()) {
			...
			String update = "update fornecedor set nome=?,cnpj=?,fone=?,cep=?,endereco=?,numero=?,complemento=?,bairro=?,cidade=?,uf=? where idfor=?";
			try {
				con = dao.conectar();
	...
	private void Excluir() {
		int confirmar = JOptionPane.showConfirmDialog(null, "Confirma a exclusão deste Fornecedor?", "Atenção!",
				JOptionPane.YES_NO_OPTION);
		if (confirmar == JOptionPane.YES_OPTION) {
			// CRUD - Delete
			String delete = "delete from fornecedor where idfor=?";
